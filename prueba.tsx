...
export default function DepartmentsPage({ data }: { data: any }) {
  ...
  return (
    <div id='distribucion' className='flex min-h-screen flex-col items-center bg-[#ffffff] p-2 dark:bg-[#0a192f] lg:p-10'>
          ...
          <AnimatePresence mode='wait'>
            ...
            {mobileTab === 'map' && (
              ...
              >
                <div className="absolute top-10 flex flex-col items-center gap-2 opacity-40">
                  <MapIcon size={20} />
                  <p className="text-[10px] font-bold uppercase tracking-widest">Mapa Interactivo</p>
                </div>
                <InteractiveSVG 
            ...
            {(mobileTab === 'model' || (typeof window !== 'undefined' && window.innerWidth >= 1024)) && (
              <motion.div
                ...
                onDragEnd={onDragEnd}
              >
                <img src={currentImage} className='h-full w-full object-contain' alt='Plan' />
                ...
                {selectedModel && (
                  <div className='absolute bottom-6 left-6 right-6 flex flex-col gap-4 lg:bottom-12 lg:left-12 lg:right-12 lg:flex-row lg:items-end lg:justify-between'>
                    <div className='rounded-2xl border border-white/10 bg-black/60 p-5 backdrop-blur-md lg:rounded-3xl lg:p-8'>
                      <p className='mb-1 text-[9px] font-bold uppercase tracking-[0.4em] text-white/70'>Detalles</p>
                      <h1 className='mb-1 font-serif text-2xl text-white lg:text-5xl'>{selectedModel.name_model_department}</h1>
                      <p className='text-sm text-zinc-300 lg:text-2xl'>{selectedModel.base_square_meters} m² totales</p>
                    </div>

                    {galleryImages.length > 0 && (
                      <Button onPress={onOpen} className='flex h-12 gap-3 rounded-xl bg-white px-6 text-xs font-bold text-black shadow-2xl transition-all hover:scale-105 lg:h-20 lg:rounded-3xl lg:px-10 lg:text-lg'>
        ...
        <aside className='hidden h-full flex-col gap-6 lg:flex lg:w-[30%]'>
          <div className='relative h-full overflow-hidden rounded-[40px] border border-white/5 bg-white shadow-2xl dark:bg-transparent'>
            <div className='absolute left-8 top-6 z-10 rounded-2xl border border-zinc-200 bg-zinc-100 px-4 py-2 dark:border-zinc-700 dark:bg-zinc-800'>
              <p className='text-[10px] font-black uppercase tracking-widest text-zinc-500 dark:text-zinc-300'>Mapa Interactivo</p>
            </div>
            <InteractiveSVG 
              svgUrl={data.plan_image} 
              departments={data.models} 
      ...
      <style jsx global>{`
        .interactive-svg-container svg { width: 100%; height: 100%; padding: 20px; }
        .interactive-svg-container [id] { cursor: pointer; transition: all 0.3s ease; stroke: #5c5c5c; fill: rgba(255, 255, 255, 0.99); stroke-width: 1px; }
        .interactive-svg-container [id]:hover { fill: rgba(0, 100, 139, 0.2) !important; stroke: #000000; stroke-width: 2px; }
        :is(.dark) .interactive-svg-container [id] { stroke: #a1a1a1; fill: rgba(30, 30, 30, 0.95); }
        :is(.dark) .interactive-svg-container [id]:hover { fill: rgba(0, 150, 180, 0.3) !important; stroke: #ffffff; stroke-width: 2px; }
        ${selectedModel ? `
