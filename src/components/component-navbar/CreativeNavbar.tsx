    ...
    <>
      ...
      >
        <div className="flex w-full max-w-6xl items-center justify-between rounded-full border border-white/20 bg-white/70 px-6 py-3 shadow-lg backdrop-blur-xl dark:border-zinc-800/50 dark:bg-black/60">
          ...
          <Link href="/" className="relative flex h-10 w-auto items-center overflow-hidden rounded-full">
            <Image
              src={LOGO_SRC}
              alt="Logo"
              width={120}
                  ...
                  {link.dropdown ? (
                    <div>
                      <button
                        ...
                        onBlur={() => setTimeout(() => setIsInicioDropdownOpen(false), 200)}
                        className="flex items-center gap-1 transition-colors hover:text-amber-500 focus:outline-none"
                      >
                        {link.title}
                        <motion.span animate={{ rotate: isInicioDropdownOpen ? 180 : 0 }}>
                          <FiChevronDown />
                        </motion.span>
                      </button>
      ...
      <AnimatePresence>
        {isMobileMenuOpen && (
          ...
          >
            ...
            <motion.div variants={mobileItemVariants} className="p-10 text-zinc-400">
