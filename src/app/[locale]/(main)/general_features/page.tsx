...
import {
  ...
  Button, Input, Modal, ModalContent, ModalHeader, 
  ModalBody, ModalFooter, useDisclosure, addToast, Spinner
} from '@heroui/react';
...
export default function GeneralFeaturesPage() {
  ...
  const createMutation = useMutation({
    ...
    onSuccess: (res) => {
      if (res.success) {
        addToast({ title: "Exito", description: "Caracteristica guardada en el catalogo", color: "success" });
    ...
    <div className="p-8 space-y-6">
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-default-500 text-small">Gestiona las opciones disponibles para los edificios</p>
        </div>
      ...
      <Table aria-label="Catalogo de caracteristicas generales">
        ...
        <TableBody emptyContent={"No hay caracteristicas creadas"}>
          {(features ?? []).map((f) => (
            <TableRow key={f.id}>
              <TableCell className="text-tiny text-default-400">{f.id}</TableCell>
              <TableCell className="font-medium">{f.name_gfeatures}</TableCell>
              <TableCell>{f.room || <span className="text-default-300 italic">General</span>}</TableCell>
              <TableCell>
        ...
        <ModalContent>
          ...
          <ModalBody className="space-y-4">
            <p className="text-small text-default-400">
              Estas caracteristicas podran ser seleccionadas luego al crear o editar un edificio.
            </p>
            <Input 
              label="Nombre" 
